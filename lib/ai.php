...
function handle_ai_chat(): void {
    ...
    if ($mode === 'reader') {
        $system = '你是一个阅读助手。用户在阅读一篇文章时选中了一段文字并提出了疑问。请根据选中的文字内容帮助用户理解、分析或解答问题。规则：
- 你是文章的读者和阐释者，不要建议修改原文。
- 如果问题涉及某个概念、典故、背景知识，请解释清楚。
- 如果问题是关于文意、情感、写作手法的分析，请给出有洞见的解读。
- 如果问题比较模糊，请尝试揣摩作者意图和上下文来回应。
- 用温和、有深度的语气回答，控制在200字以内。';
    } else {
        $system = "你是一个 Markdown 编辑器中的写作助手。用户正在编辑文章原文，你看到的内容是纯文本 Markdown 源码。重要规则：
